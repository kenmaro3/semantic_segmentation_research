...
class Data(Dataset):
    def __init__(self, paths_data, paths_label, obj_data, obj_label, frames) -> None:
        ...
        self.datas = np.zeros((self.data_size, 3, self.crop_size, self.crop_size))
        self.labels = np.zeros((self.data_size, 2, self.crop_size, self.crop_size))
        for i in range(self.data_size):
            video_index = random.randint(0, len(self.paths_data)-1)
            target_video = self.paths_data[video_index]
            target_label = self.paths_label[video_index]

            frame_one = np.zeros((3, self.crop_size, self.crop_size))
            label_one = np.zeros((1, self.crop_size, self.crop_size), dtype=np.int32)
            ...
            ret_, frame_label = cv.threshold(frame_label, 50, 255, cv.THRESH_BINARY)
            label = np.where(frame_label < 50.0, 0, 1)
            label_reverse = np.ones_like(label) - label
            ...
            crop_x = np.random.randint(0, frame.shape[1]-self.crop_size-1)
            crop_y = np.random.randint(0, frame.shape[0]-self.crop_size-1)
            # print(f"crop_y: {crop_y}")
            # print(f"crop_x: {crop_x}")
            # original_frame = frame
            # cv.imwrite('original.png', original_frame)
            tmp_frame = frame[crop_y:crop_y+self.crop_size, crop_x:crop_x+self.crop_size, :]
            frame_one = np.transpose(tmp_frame, (2, 0, 1))
            tmp_label = label[crop_y:crop_y+self.crop_size, crop_x:crop_x+self.crop_size]
            tmp_label_reverse = label_reverse[crop_y:crop_y+self.crop_size, crop_x:crop_x+self.crop_size]
            frame_label = tmp_label
            frame_label_reverse = tmp_label_reverse

            self.datas[i] = frame_one
            self.labels[i, 0, :, :] = frame_label
            self.labels[i, 1, :, :] = frame_label_reverse
