...
class DataLoader(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        batch_frame = np.zeros((self.batch_size, 224, 224, 3))
        batch_label = np.zeros((self.batch_size, 224, 224))
        ...
        for i in range(self.batch_size):
            ...
            crop_x = np.random.randint(0, frame.shape[1]-224-1)
            crop_y = np.random.randint(0, frame.shape[0]-224-1)
            # print(f"crop_y: {crop_y}")
            # print(f"crop_x: {crop_x}")
            # original_frame = frame
            # cv.imwrite('original.png', original_frame)
            frame = frame[crop_y:crop_y+224, crop_x:crop_x+224, :]
            label = label[crop_y:crop_y+224, crop_x:crop_x+224]
        ...
        assert batch_frame.shape == (self.batch_size, 224, 224, 3), f"{batch_frame.shape}"
        assert batch_label.shape == (self.batch_size, 224, 224), f"{batch_label.shape}"
